...
int main(int argc, char * argv[])
{
  ...
  desc.add_options()
      ...
      ("num-bins,n", po::value<unsigned > (&nBin)->default_value (20), "number bins for distribution.")
  ...
  for (unsigned ii = 0; ii < nBin; ++ii) {
    ...
    double vv = avgs[ii].getAvg() / double(countTot) * binSizeI;
    double ee = avgs[ii].getAvgError() / double(countTot) * binSizeI;
    fprintf (fp, "%f %e %e\n", xx, vv, ee);
  }
