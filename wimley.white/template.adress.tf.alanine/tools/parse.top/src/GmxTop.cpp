...
void GmxTop::
convertType_3_1 (const gmx_sys_types	& old_types,
		 gmx_sys_types		& new_types)
{
  if (old_types.defaults.comb_rule != 3){
    cout << "old comb rule is not 3, return without doing anything" <<endl;
    return ;
  }
...
bool GmxTop::
matchAtomName (const string & name,
	       const gmx_sys_top & systop,
	       const gmx_sys_types & systypes,
	       gmx_atomtypes_item & atomtype)
{
  bool find = false;
  ...
  for (unsigned jj = 0; jj < systop.moles[ii].atoms.size(); ++jj){
    if (name == systop.moles[ii].atoms[jj].at_name){
      myatom = systop.moles[ii].atoms[jj];
      find = true;
    }
  }
  }
  ...
  if (!matchAtomType (myatom.at_type, systypes, atomtype)) {
    return false;
  }
  else {
    return true;
