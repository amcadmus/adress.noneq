...
{
  std::string ofile, ifile0, ifile1;
  ...
  desc.add_options()
      ...
      ("input-dist-0", po::value<string > (&ifile0)->default_value ("0.out"), "the traj name 0.")
      ("input-dist-1", po::value<string > (&ifile1)->default_value ("1.out"), "the traj name 1.")
  ...
  while (fpname0.getline(nameline0, MaxLineLength) &&
    ...
    for (unsigned ii = 1; ii < words.size(); ++ii){
      dist1.push_back (atof(words[ii].c_str()));
    }
    if (time0 != time1){
