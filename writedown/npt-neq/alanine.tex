...
In this perspective, if one wants to use MD, the proper way to do it would be that of treating a situation of nonequilibrium with algorithms that can explicitly describe it. This is the approach that will be followed in this work.
...
changes the conformation in an EF (conformational space), when do these changes happens (time scale),
and how likely they are (probability). 
To our knowledge, this study is one of the few (if not the first)  where nonequilibrium w.r.t. conformation is explicitly considered for a large molecule in solution subject to time-dependent EF.
...
Stochastic dynamics (Langevin) can be hadeled analogoursly.
...
As it is well known, in MD, for equilibrium situations, the solution is to couple
...
conditions.
However, coupling the system to a thermostat for nonequilibrium situations is a delicate issue.
In fact, since a thermostat is explicitly designed to bring the system into equilibrium, any perturbation produced by the EF would be mostly adsorbed by the thermostat. This means that the physics of the original Hamiltonian dynamics with the EF would be lost and the response to the perturbation becomes unphysical. 
...
Therefore, a realistic nonequilibrium  dynamics is preserved in the Hamiltonian dynamics region (where the properties of interest are observed)
...
the warm-up time $t=t_{\textrm{warm}}$, the eclectic field is switched on linearly, while
...
\bibitem{bohner2012algorithm}
Matthias~U Bohner and Johannes K{\"a}stner.
\newblock An algorithm to find minimum free-energy paths using umbrella
  integration.
\newblock {\em The Journal of Chemical Physics}, 137:034105, 2012.

\bibitem{seifert2010fluctuation}
U.~Seifert and T.~Speck.
\newblock Fluctuation-dissipation theorem in nonequilibrium steady states.
\newblock {\em EPL (Europhysics Letters)}, 89:10007, 2010.
