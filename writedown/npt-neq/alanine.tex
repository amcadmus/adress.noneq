...
\begin{table}
  ...
  \begin{tabular*}{0.4\textwidth}{@{\extracolsep{\fill}}lc}\hline\hline
    ...
    $A_1\rightarrow A_2$        &       $0\sim 10$      \\
    $B_1\rightarrow B_2$        &       $0\sim 10$      \\    
    $A_2\rightarrow A_1$        &       $10\sim 100$      \\
    $B_2\rightarrow B_1$        &       $10\sim 100$      \\    
    $B_2\rightarrow A_1,\ A_2,\ C$        &       $10\sim 50$      \\    
    $B_1\rightarrow A_1,\ C$        &       $10\sim 500$      \\    \hline\hline
  \end{tabular*}
...
the EF is switched on ($t_{\textrm{init}} = 10~\textsf{ps}$).
...
between systems with and without the EF, and the
...
conformational changes under the EF.
...
indicate if the molecule is perpendicular to the EF.
...
If the molecule is perfectly parallel to the EF, then $S_\theta = -2$.
...
for $T=10$ and 40~ps, as the EF changes faster,
...
metastable state for periodically oscillating EF.  The
