...
\affiliation{Dipartimento di Fisica, Università “La Sapienza” and CNISM, Piazzale Aldo Moro 5, 00185 Roma, Italy}
...
\recheck{where $U^\dagger(t,0) = \mathcal T\exp\{-i \int_0^t d t' L(t')\}$, and $\mathcal T$ is the time ordering operator.}
On the other hand,
\begin{align}
...
\begin{figure}
  ...
  \caption{
    ...
    \recheck{Considering the angular periodicity, The angle, for example, $226^\circ$ corresponds to $-134^\circ$
    on the plot (b) and (c).}
...
\end{align}}
\recheck{\textit{I cannot read what Giovanni wrote on the top of page 9.}}
...
\begin{figure}
  ...
  \caption{The
    ...
    function of time. \recheck{The solid line represents the dipole moment,
...
\begin{align}\label{eqn:tmp8}
  ...
  \bigg\rangle,
\end{align}
\recheck{where $q_i$ denotes the partial charge of any point particle $i$ defining our model of alnine,
...
the warm-up time $t_{warm}$, and is 25 times \recheck{\textit{25 time should be correct, isn't it?}}
...
% \bluec{\textit{Here I do not agree with you to remove the above blue discussion,
%   because the time scales (also the oscillatory EF case) are
%   the main results of the current paper, which should be presented. It is only one sentence, no one would be panic.}}
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\textwidth]{graph-t0010-2.eps}
  \caption{Schematic plot of the main probability flux between conformations
    ...
    % {\bf LUIGI COMMENT: THE PICTURE MUST BE MUCH MUCH LAREGER SO THAT ONE SEE THE DIFFERENCE IN THE STRUCTURES. mOREOVER, THEY HAVE AN INTEREST ONLY THEY CAN BE RELATED TO MORE GENERAL STRUCTURES. wHAT IS NEEDED IS THAT CLOSE TO EACH MOLECULE ONE SHOULD WRITE ``$\alpha$-helix-like'' ``$\beta$-sheet-like. THE REST, gIOVANNI IS RIGHT, DOES NOT NEED TO BE EXPALINED IN DETAIL BECAUSE IT IS NOT REALLY INTERESTING AND i CUT A LOT.}
  }
  \label{fig:tmp11}
\end{figure}
...
\recheck{\textit{I think it is interesting because such directional probability
  flux network is firstly reported by us. How to revise this part to convince Giovanni?}}
