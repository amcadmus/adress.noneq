...
\recheck{
  In the appendix of the paper, we also compare our nonequilibrium method with
  the equilibrium linear response theory,
  which is able to study the nonequilibrium properties
  by only calculating equilibrium correlation functions.}
...
\recheck{ The definition of the probability flux is similar to the
  ...
  % Since the main goal of our research is to propose a
  % nonequilibrium molecular dynamics simulation method, the numerical
  % results will not be compared with the kinetic analysis.
}
...
\recheck{
  Although the network presented Fig.~\ref{fig:tmp11} is similar to those
  ...
  the fluxes are lasting. In contrast,
...
\recheck{
A commenly used equilibrium method that is able to study the nonequilibrium
responses is the linear responses thoery (Green-Kubo relation~\cite{green1954markoff,kubo1957statistical}).
We also use it to test the
conformational dynamics of alanine dipeptide under the EF.
The response function, take the time-dependent probability of
left-handed $\alpha$-helix for example, is defined as~\cite{tuckeman2010statistical}
...
otherwise. $M_e(t)$ is the relative magnitude of the EF. Take the case of
...
$\langle j(0)\cdot \chi_{\confc}(t) \rangle_0$ against corrlation time $t$
in Fig.~\ref{fig:appdx1}. The value of the function is estimated from
two independent 1~$\mu$s equilibrium simulations.
Please notice that the total length of the equilibrium
trajactories is the same as the nonequilibrium simulation (2000
trajactories, each 1000~ps long).
% With the knowledge of
% nonequilibrium simulation, the correlation function is expected to
...
simulation, we conclude that nonequilibrium simulatoin is more
precise than the conventional equilibrium approach (linear response) at the same computational cost.
A similar
observation has already been reported by Ciccotti and Jacucci~\cite{ciccotti1975direct}.
}

\begin{figure}
  \centering
  \includegraphics{fig.ala.new/fig-corr-meta.eps}
  \caption{The corrlation function $\langle j(0)\cdot \chi_{\confc}(t)
