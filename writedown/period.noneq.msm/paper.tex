...
details that \recheck{sometimes} cannot be resolved in experiments. Only
recently, a \recheck{dynamical} non-equilibrium MD simulation (D-NEMD) method with local thermostating has been proposed~\cite{wang2014exploring} that allows for studying 
...
non-equilibrium \recheck{MD} simulations of alanine dipeptide under oscillating EF.
...
\label{sec:disc}
\recheck{We consider diffusive molecular dynamics} in an energy landscape $V$ driven by the time-dependent external driving force $E(t)D(x_t)$ with the
...
\recheck{The Hamiltonain or Langevin dynamics can be treated in the same way as that will be presented, so for sake of simplicity we focus on the discussion of diffusive dynamics.}
...
We will now introduce an appropriate spatial discretization of this kind of non-equilibrium MD -- \recheck{\bluec{(why saying this red sentence? we need the spatial discretization for calculating the core sets...)} this is done for reasons of simplicity only; we could completely avoid it for the price of more technical arguments.}
...
satisfying
$\mymu^{\top}P=\mymu^{\top}$.
From now on, we assume $P$ to be irreducible and aperiodic such that the Perron-Frobenius theorem \recheck{holds, so the eigenvector corresponding to} the eigenvalue $\lambda=1$ is  non-negative componentwise, and unique (up to normalization $\sum_i\mu(i)=1$). In this case $\mymu$ is the stationary measure in the sense that
$
...
\recheck{The Gromacs default Fourier spacing of 0.12~nm and B-spline interpolation order of 4 were adopted.}
The splitting
...
For this periodically driven molecular system we will first show how to choose an appropriately fine \recheck{spatial} discretization. After validating this discretization we will consider
the \recheck{temporal} discretized dynamics generated by the Floquet transition matrix $\vect P$
...
the rest of the trajectory in time interval $[320,4000]$~ps is \recheck{averaged} to estimate
...
\begin{figure}
  ...
  \vskip -.5cm
  \caption{\recheck{The forward $q^\fwd_{\confaa}$ and backward 
...
\begin{figure}
  ...
  \vskip -.5cm
  \caption{\recheck{The forward $q^\fwd_{\beta}$ and backward
...
\begin{figure}
  ...
  \vskip -.5cm
  \caption{\recheck{The forward $q^\fwd_{\confc}$ and backward $q^\bwd_{\confc}$
    committors
...
\end{align}
where $p(i,t)=\prob(X_t=i)$ is the probability to be in set $\Omega_i$ at time $t$ as governed by the Master equation, and the observable $\mathcal A$ is \recheck{linearly} spanned by the backward
committors, i.e.,
\begin{align}\label{A}
...
\begin{figure}
  \centering
  \includegraphics[width=0.48\textwidth]{fig/flowchart/flowchart2.eps}
