...
The non-equilibrium biomolecular systems, especially
those systems under periodic driven, are attracting increasing
interest.  For example, the potential effects of an electromagnetic
radiation on the human body tissue (e.g.~DNA, protein, and membrane)  has been extensively
investigated by a vast number of literatures, and a very limited
subset of them is Ref.~\cite{bohr2000microwave, bohr2000microwave-1, dePomerai2000cell,
  ...
  damm2012can, english2009nonequilibrium, solomentsev2012effects,
  wang2014exploring}.  Among those theoretical investigations, the
molecular dynamics (MD) has been shown  useful in understanding
the response of the biomolecular conformation to the external electric field,
due to its ability of resolving the molecular
details, which are sometimes not achievable in experiments. Only
recently, a dynamical non-equilibrium MD simulation (D-NEMD) method has
been proposed.  With the local thermostating technique, the D-NEMD
method is able to sample the non-equilibrium probability density at the
constant temperature, therefore opens the possibility of studying the
non-equilibrium processes in a statistically reliable and
thermodynamically consistent way. 

% \textbf{MSM tools for analyzing, provide profound understanding.}

Despite their high resolution in presenting a molecular system, MD the
trajectories including the time-dependent position and velocity of every atom
sometimes impose non-trivial challenge for data analyzing, in the sense that 
extracting the essential structure and dynamical properties is
becoming increasingly difficult as the  system size and trajectory length grow.
% by merely visulizing (not only molecular cartoons but also order parameters)
% and observing the molecules.
The Markov state model
(MSM)~\cite{A19-31}, which is a promising method for data analyzing of
the high-dimensional MD trajectories, has been well developed over the
past decade in theory and applications \cite{A19-31, A19-1}, and in
the software implementations~\cite{A19-49, MSMBuilder}.  The principal
idea of MSM is to approximate the original high-dimensional system by
a reduced Markovian dynamics over a finite number of states, from
which the essential structures and dynamics can be formulated by the spectrum analysis.
By assigning the spacial and temporal discretizations, 
the transition probabilities are sampled from MD trajectories, 
% the MSM does not need more information from the MD simulation.
so the quality of the MSM  relies on the quality of the discretizations.
Moreover, the characteristic of the original dynamics plays a role:
It has been shown that in principle good approximate accuracy would be achieved
by using only a few ``coresets'',
if the system is dominated by a few metastable states, in the sense that
the MD trajectories stay in the vicinity of a metastable state substantially longer than
transform to another such state~\cite{schuette2011markov}.
Besides the applications in analyzing the MD trajectories, it worth noting that MSM is
also used for designing initial configurations for massively parallel MD
simulations~\cite{kohlhoff2014cloud}.
...
analysis of the non-equilibrium MD simulations mainly follows the
visualizing (not only molecular cartoons but also order parameters)
and observing approach, and the quantitative description of
the essential conformational dynamics is still lacking. On the other
hand, the MSM, to the best of our knowledge, has only been applied to
equilibrium systems,  and most of them are assumed reversible. This work is the first attempt
of using the MSM in analyzing a class of truly non-equilibrium systems:
periodic external driven. Being more specifically,
we investigate the protein conformational dynamics
under the oscillating electric field (EF).
In Sec.~\ref{sec:disc}, we discuss the temporal and spacial
discretizations of the non-equilibrium dynamics. The spacial discretization
is on the dihedral angle space, which can be interpreted as a
MSM in the traditional sense of full spacial partition.
In the temporal direction, the non-equilibrium process
is  discretized by the Floquet theorem into a time-homogeneous
but not necessarily reversible dynamics. Since the full spacial partition
is not preferable in high-dimensional system, in Sec.~\ref{sec:build-msm},
the MSM based on milestoning is introduced for the discretized irreversible process.
The validity of the discretizations and the MSM is checked in Sec.~\ref{sec:alanine}
by comparing to the brute force
non-equilibrium simulation of alanine dipeptide under oscillating EF.
This work is summarized in Sec.~\ref{sec:conclusion} by a chart presenting
the workflow of building MSM for non-equilibrium systems,
and open questions are presented in the end.
...
\label{sec:disc}
For the sake of simplicity we consider diffusive molecular dynamics in an energy landscape $V$ driven by the time-dependent external driving force $E(t)D(x_t)$ with the
...
\begin{figure}
  \centering
  \includegraphics[width=0.45\textwidth]{fig/t010/discrete/fig-cg-prob.eps}  
...
\begin{figure}
  \centering
  \includegraphics[width=0.45\textwidth]{fig/t010/discrete/fig-cg-prob-tau.eps}  
...
\begin{figure}
  ...
  \caption{The forward $q^\fwd_{\confaa}$ and backward 
    ...
    computed by the NEMD simulations (left
...
\begin{figure}
  ...
  \vskip -.5cm
  \caption{The forward $q^\fwd_{\beta}$ and backward
...
\begin{figure}
  ...
  \vskip -.5cm
  \caption{The forward $q^\fwd_{\confc}$ and backward $q^\bwd_{\confc}$
    committors
...
\begin{figure}
  \centering
  \includegraphics[width=0.48\textwidth]{fig/flowchart/flowchart.eps}
  \caption{The flowchart of this work. A lighter color indicates
  a ``coarser'' approximation to the original NEMD dynamics.}
  \label{fig:flowchart}
\end{figure}

In this paper we developed methods of building MSM for a periodically driven
...
both temporally and spatially, and the end-product is a time-homogeneous
...
i.e.~firstly spacial and then temporal,
or a single step  involves both discretizations simultaneously.
...
In principle most of the discretization methods for the equilibrium
case can be generalized to non-equilibrium without substantial
difficulty, however, we have not compare different discretization methods
...
discretization over the dihedral angles $\phi$--$\psi$ space is a good
...
The Floquet theorem discretize a periodic process by taking only
...
The advantage of this discretization is  obvious: All
well-developed theories and computational tools for time-homogeneous
Markov processes can be directly applied. In the alanine dipeptide
...
Although the end-product of the two-step and one-step discretizations are equivalent,
...
whenever the long-time behaviors of the system is of interest, and
...
discretization is preferred, so that one can recover the dynamics within
one period no matter when it is necessary. If the longest non-trivial
time-scale is comparable to one period, then the Floquet theorem
is not preferable, and the non-equilibrium dynamics may be well
described by the quasi-equilibrium approximation.

Building the MSM from the time-homogeneous discretized dynamics is straightforward by
using Eq.~\eqref{eq:msm-tmatrix-00} and a set of coresets that are
derived from the discretized dynamics.  Numerical
results shows that a 3-state MSM can reproduce the leading non-trivial
...
accuracy, the time-evolution of the metastable state probabilities from
equilibrium (no external driven) to the non-equilibrium
...
simplified when an accurate time-homogeneous discretization 
...
numerical example itself presents interesting phenomena: under the
oscillatory EF, the population of the left-handed $\alpha$-helix
significantly increases with respect to the equilibrium population
(see also the discussions in Ref.~\cite{wang2014exploring}),
and the leading time-scale is much shorter than the equilibrium case.
...
(the equilibrium counterpart is provided by Ref.~\cite{schutte2014markov}).
