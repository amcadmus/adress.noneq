...
\end{align}
where $\vect L^T(t)$ denotes the matrix transpose of $\vect L(t)$, $\vect p(t)$ is an $N$-vector denoting the probability distribution on $S$ at time $t$, $p_i(t)$, for example, the probability to be in state $i$ (which corresponds to set $\Omega_i$) at time $t$.
...
\recheck{COMMENT: Here it is a \emph{conditional} probability,
...
\begin{figure}
  ...
  \caption{The color scale plot of the logarithmic quasi-stationary distribution $\mymu$
    of (a) the NEMD  and (b)
    the discretized dynamics governed by Floquet transition matrix $\vect P=\myphi^T(T)$.
  }
  \label{fig:num-1}
\end{figure}
...
\[
(\vect P-\id) h_{C_j}(i) = -1,\qquad \mathrm{if} \, C\cap \Omega_i=\emptyset.
\] 
...
The leading eigenvalues of $P=\myphi^T(T)$ are compared with those of $\hat P$ and $\hat{T}\hat{M}^{-1}$ in
Tab.~\ref{tab:tmp1}.
...
time-scale is not as good as the first, but is still acceptable (even good for $\hat{T}\hat{M}^{-1}$).
The reason
...
\begin{figure}
  ...
  \caption{Comparison of NEMD and MSM long-term kinetics of alanine
    ...
    time-dependent probability $p_j=\hat p(j,t)$ to be assigned to
    core set $C_j$ (corresponding to the observable $\mathcal A$ given
    in (\ref{A}) with $\alpha_j=1$ and $\alpha_i=0$ for $i\not=
...
In the NEMD case, the backward committor and the probability density $\hat p_j (mT) = \sum_{i\in S}  q^\bwd_j(i)  p(i,mT) $
