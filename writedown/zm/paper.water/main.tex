...
Introduction of the ZMM, formula...[ {\color{blue} COMMENT: I can
write here later}]
...
In order to investigate the physical-chemical properties obtained  by ZMM, we simulate the water
system by using both SPME and ZMM, and evaluate a set of \emph{%
...
computing the benchmark quantities should indicate a good accuracy of the ZMM.
More importantly, the benchmark tests provide knowledge in the
situations when the ZMM does not work so well, and help in finding the
way of tuning the working parameters to achieve higher accuracy. The ZMM
is also compared to the traditional short-range method: RF.
...
too large to show no convergence to 1~Bar. This implies that $\alpha \leq 1\,\text{nm}^{-1}$
...
the NPT simulations) of roughly 0.1\%. Taking different orders ZMM  at
