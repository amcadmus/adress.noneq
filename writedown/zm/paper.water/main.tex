...
%TCIDATA{LastRevised=Tuesday, November 11, 2014 22:18:30}
...
cut-off method. the traditional gromacs uses gropu cut-off as default. We
...
[\recheck{please see the red part in Sec. III B. The order of the paragraphs are also rearranged.}]
...
which means the system is in desired thermodynamic state. [ {\color{blue}
COMMENT: If there are any specific conditions only for the pressure
calculation, please state around here or elsewhere.] [\recheck{No.}]}
...
when using increasingly large cut-off radius. In Fig.~\ref%
{fig:gkr-damp-l1}, it shows that in the case of $l=1$, the largest splitting
parameter ($\alpha =2.0\ \text{nm}^{-1}$) is worse in calculating the
Kirkwood-G factor than smaller splitting parameters.
Splitting parameter dependency was not so clear for $l=2$ and 3 (shown in
SI). Note that in many cases the discrepancies from the SPME were relatively
prominent in the middle range of the distance and they tend small for larger
distance.
